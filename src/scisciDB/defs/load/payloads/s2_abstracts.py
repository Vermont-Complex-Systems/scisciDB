"""
Payload for s2_abstracts asset.
...
def main(context):
    ...
    source_dir = os.path.join(s2_data_root, "abstracts")
    ...
    try:
        ...
        context.log.info(f"Creating s2_abstracts view over {source_dir}...")
        t0 = time.time()

        conn.execute("DROP VIEW IF EXISTS s2_abstracts")
        conn.execute(f"""
            CREATE VIEW s2_abstracts AS
