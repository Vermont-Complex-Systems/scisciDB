"""
Payload for s2_has_abstract asset.

Adds has_abstract boolean column to s2_papers by checking whether
corpusid exists in s2_abstracts. Incremental by default.
"""
import os
import shutil
import time
...
def main(context):
    ...
    try:
        ...
        conn.execute("ALTER TABLE s2_papers ADD COLUMN IF NOT EXISTS has_abstract BOOLEAN")

        where_clause = "" if force_update else "AND p.has_abstract IS NULL"
        if force_update:
            context.log.warning("force_update=True: re-checking ALL s2_papers (expensive)")
        else:
            context.log.info("Incremental mode: only updating papers where has_abstract IS NULL")

        context.log.info("Updating has_abstract...")
        t0 = time.time()

        result = conn.execute(f"""
            UPDATE s2_papers AS p
            SET has_abstract = TRUE
            FROM s2_abstracts a
            WHERE p.corpusid = a.corpusid
              {where_clause}
        ...
        context.log.info(f"has_abstract: {updated:,} rows set to TRUE in {elapsed:.1f}s")

        # Set remaining NULLs to FALSE
        conn.execute("UPDATE s2_papers SET has_abstract = FALSE WHERE has_abstract IS NULL")

        total = conn.execute("SELECT COUNT(*) FROM s2_papers").fetchone()[0]
        n_abstract = conn.execute(
            "SELECT COUNT(*) FROM s2_papers WHERE has_abstract = TRUE"
        ).fetchone()[0]

        context.log.info(
            f"has_abstract: {n_abstract:,}/{total:,} ({n_abstract/total*100:.1f}%) — mode={mode}"
        )

        context.report_asset_materialization(metadata={
            "rows_updated": updated,
            "total_with_abstract": n_abstract,
